...
class AccountAction extends BaseAction {
    function register() {//注册页面显示
        $this->toclose();
        $this->uid != NULL && $this->redirect('Index/index');//用户已登录则跳转
        $this->assign('title', '注册本站');
        $this->display();
    }
    function login() {//登录页面显示
        $this->uid != NULL && $this->redirect('Index/index');//用户已登录则跳转
        $this->assign('title', '登录本站');
        $this->display();
    }
    private function toclose() {//当系统设置为关闭注册时跳转至关闭注册页面
        if ($this->setting['register_close'] == 1) {
            $this->redirect('Account/registerclose');
            exit;
        }
    }
    function registerclose() {//关闭注册页面显示
        ($this->uid != NULL || $this->setting['register_close'] != 1) && $this->redirect('Index/index');
        $this->assign('title', '站点注册已关闭');
        $this->display();
    }
    function ajax_check_name() //ajax 验证，用于注册时的验证
    {
    ...
    private function check_register() //注册时第二次验证，防止非法提交
    {
        $this->uid != NULL && exit('Acess Denied!');
        $this->toclose();
        $register = M('user');
    ...
    function verify_receive() {//用户打开发往邮箱中的激活链接时触发的函数
        $this->uid != NULL && exit('Acess Denied!');
        $this->toclose();
    ...
    function register_submit() {//注册时提交至服务器处理函数
    ...
    function ajax_login() {//ajax登录服务器处理，
        $this->uid != NULL && exit('Access Denied!');
        $user = M('user');
        ...
        if ($this->setting['ucenter_on'] == 1) {//ucenter开启时，开始执行ucenter登录
    ...
    function activate() {//当用户在ucenter中存在而不再本地数据库中存在时，跳转到激活页面
		$this->uid != NULL && exit('Access Denied!');
        $this->toclose();
    ...
    function activate_receive() {//激活时服务器接收数据
		$this->uid != NULL && exit('Access Denied!');
        $this->toclose();
    ...
    function logout() {//注销登录，包括清除cookie，session以及ucenter注销
    ...
    function logoutnew() {//注销登录，包括清除cookie，session以及ucenter注销
        setcookie('qcs_auth', NULL, time() - 1, '/');
		Session::set('uid',NULL);
                Session::set('type',NULL);
        if ($this->setting['ucenter_on'] == 1) {
