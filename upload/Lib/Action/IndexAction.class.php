...
class IndexAction extends BaseAction
{
    ...
    function all()
		{
			$count=NULL;
			if($this->uid!=NULL)//如果用户未登录则显示全部问题，否则显示所有问题
				{
					$focus=M('focus');
	...
	function focus_question()//我关注的问题
		{
			!$this->isAjax()&&exit;
			if($this->uid==NULL){
				exit('参数错误！');
	...
	private function print_list($question_list,$count)//输出问题列表函数
		{
			if($question_list==NULL)
					{
	...
	function recommend_question()//推荐问题的展示
		{
			!$this->isAjax()&&exit;
			$question=M('question');
			if($this->get['cid']==NULL)
