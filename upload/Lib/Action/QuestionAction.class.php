...
class QuestionAction extends BaseAction {
    private function tologin() {//未登录时，则阻止后续程序运行
        $this->uid == NULL && exit($_POST['c'] . '<script>tip("请先登录");</script>');
    }
    function publish() {//发布问题页面的输出
        if ($this->uid == NULL) {
    ...
    function receive_ini() {//问题发布时初始化，提取关键词
        $this->tologin();
        $kwd = get_keywords($this->post['title']);
        echo $kwd ? implode(",", $kwd) : '';
    }
    function receive() {//问题发布时，服务器端处理
        $this->tologin();
    ...
    function editdone() {//编辑完毕后，服务器端处理函数
        $this->tologin();
        $data['title'] = F('banwords') ? (strtr($this->post['pub_title'], F('banwords'))) : ($this->post['pub_title']);
    ...
    function set_best_answer() {//设为最佳答案函数
        $question = M('question');
    ...
    function view() {//问题浏览页面
        ...
        if (in_array($this->uid, $focus_arr) && $focus_id[0]['uid'] != NULL) {//判断我是否关注了这个问题
            $f->where(array(
                'qid' => $this->get['qid'],
                'uid' => $this->uid
    ...
    function add_answer() {//添加回复时，服务器端处理
        ...
        if (!$ai = $answer->add($data)) {
            ...
            if ($this->post['quid'] != $this->uid) {//添加新通知
                $user->where(array(
                    'id' => $this->post['quid']
                ))->setInc('newnotice');
            ...
            if ($this->post['extra'] == 1 && $this->post['extra_auid'] != $this->post['quid']) {//如果是回复别人的回复
                $user->where(array(
    ...
    function useless() {//用户认为该回复无帮助的处理
        $this->tologin();
        $data['aid'] = $this->post['aid'];
        $data['uid'] = $this->post['auid'];
        $data['actionid'] = $this->uid;
    ...
    function invite_search() {//用户输入邀请人名时的，输出ajax下拉框
        $map['name'] = array(
            'like',
            $this->get['q'] . '%'
        );
    ...
    function search_question() {//用户在顶部搜索问题时的ajax下拉框，执行的LIKE搜索
    ...
    function reply_upload() {//用户回复上传附件，以JSON形式返回
    ...
    function reply_upload_delete() {//删除附件时的操作，同时在服务器中删除该附件
        $this->uid == NULL && exit('尚未登录');
        unlink('Public/upload/' . $this->post['name']);
    }
    function search() {//用户输入问题后，点击搜索后服务器执行的搜索
        $question = M('question');
    ...
    function search_topic() {//在话题栏内输入时执行的ajax下拉框输出
        $map['name'] = array(
            'like',
            $this->get['q'] . '%'
        );
