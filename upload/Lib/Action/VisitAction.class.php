...
class VisitAction extends BaseAction {
    ...
    private function pro() {//其他用户的展示页面时判断该用户是否存在
    ...
    private function mylist($my, $no_list) {//输出列表页面
        $arr = array();
        foreach ($my as $k => $v) {
    ...
    function hisagainst() {//其他用户的反对的问题
        $this->pro();
        $a = M('replyagainst');
        $my = $a->where(array(
            'actionid' => $this->uid
    ...
    function hisrecommend() {//其他用户的推荐的问题
        $this->pro();
        $a = M('recommend');
        $my = $a->where(array(
            'uid' => $this->get['uid']
