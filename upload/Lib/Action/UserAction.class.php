...
class UserAction extends BaseAction {
    ...
    private function tolog() {//用户没登录则跳转至登录界面
        $this->uid == NULL && $this->redirect('Account/login');
    }
    function avatar() {//用户头像显示页面
        $this->tolog();
        $this->assign('title', '头像设置');
        $this->display();
    }
    function profile_save() {//用户中心首页保存
        $this->tolog();
        array_pop($this->post);
        M('user')->where(array(
            'id' => $this->uid
    ...
    function send_letter() {//全局发送站内信的处理
        $this->tolog();
    ...
    function letterview() {//站内信的详细对话内容展示
        $this->tolog();
    ...
    function letter_reply() {//回复别人的站内信
        $this->tolog();
    ...
    private function mylist($my, $no_list) {//输出关于我的问题，如回复的、赞同的等等
        $arr = array();
        foreach ($my as $k => $v) {
    ...
    function user_search() {//输入用户名时的ajax下拉框输出
